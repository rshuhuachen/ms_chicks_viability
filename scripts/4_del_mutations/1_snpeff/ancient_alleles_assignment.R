...
data_anc <- read_tsv("/vol/cluster-data/rchen/git/ms_purging_grouse/data/processed/ancestral_66_ltet_snps.tsv.gz", skip = 1, col_names = F) 
...
data_snps <- read_tsv("/vol/cluster-data/rchen/git/ms_purging_grouse/data/processed/annotated/ltet_snps_filtered_chicks.vcf.tsv.gz")
...
tibble( type = c("div Anc / total SNPs", "miss Anc / div Anc", "miss Anc / total SNPs"),
  ...
  write_lines("/vol/cluster-data/rchen/git/ms_purging_grouse/data/processed/annotated/ancestral_allele_mismatches_chick.tex")
...
data_new_ref %>% 
  write_tsv("/vol/cluster-data/rchen/git/ms_purging_grouse/data/processed/annotated/anc_allele_assignment_chick.tsv.gz")

data_new_ref %>% 
  select(`#CHROM`, FROM = POS, TO = POS, AA) %>%
  write_tsv("/vol/cluster-data/rchen/git/ms_purging_grouse/data/processed/annotated/anc_allele_assignment_chick.bed")
